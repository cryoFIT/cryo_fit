...
def retrieve_chain(seeking_res, seeking_res_num, line_num_matching_pair):
   pdb_before_cryo_fit_in = open(pdb_before_cryo_fit)
   ori_line_num = 0
   for ori_line in pdb_before_cryo_fit_in:
      ori_line_num = ori_line_num + 1
      if int(ori_line_num) == int(line_num_matching_pair):
         if (ori_line[:4] == "ATOM") or (ori_line[:6] == "HETATM"):
            res_num = ori_line[23:27]
            if str(res) == str(seeking_res):
               if int(res_num) == int(seeking_res_num):
                  pdb_before_cryo_fit_in.close()
                  after_this_line_num_in_ori = ori_line_num
                  chain = ori_line[20:22]
                  return chain, after_this_line_num_in_ori
   pdb_before_cryo_fit_in.close()
   after_this_line_num_in_ori = ori_line_num
   return "not_retrieved", after_this_line_num_in_ori
...
for line in pdb_after_cryo_fit_in:
   ...
   if line[:5] == "CRYST" or line[:3] == "END" or line[:6] == "HETATM" or \
      ...
      retrieved_chain, after_this_line_num_in_ori_pdb = retrieve_chain(res, res_num, line_num_matching_pair)
